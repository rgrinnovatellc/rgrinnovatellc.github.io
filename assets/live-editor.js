...
const LIVE_EDITOR_CONFIG = {
  repository: 'rgrinnovatellc/rgrinnovatellc.github.io',
  apiBase: 'https://api.github.com/repos/rgrinnovatellc/rgrinnovatellc.github.io',
  defaultBranch: 'master',
  editPassword: 'YOUR_EDIT_PASSWORD_HERE' // Change this to your desired password
};
...
class LiveEditor {
  ...
  async createGitHubBranch(branchName) {
    ...
    try {
      ...
      const masterResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}/git/ref/heads/${LIVE_EDITOR_CONFIG.defaultBranch}`, {
        headers: {
      ...
      const branchResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}/git/refs`, {
        method: 'POST',
        headers: {
          'Authorization': `token ${token}`,
          'Accept': 'application/vnd.github.v3+json',
  ...
  async updateGitHubFile(filePath, content, branchName, commitMessage) {
    ...
    try {
      ...
      const fileResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}/contents/${filePath}?ref=${branchName}`, {
        headers: {
          'Authorization': `token ${token}`,
          'Accept': 'application/vnd.github.v3+json'
        }
      ...
      const updateResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}/contents/${filePath}`, {
        method: 'PUT',
        headers: {
          'Authorization': `token ${token}`,
          'Accept': 'application/vnd.github.v3+json',
  ...
  async createGitHubPullRequest(branchName, filePath) {
    ...
    const prResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}/pulls`, {
      method: 'POST',
      headers: {
        'Authorization': `token ${token}`,
        'Accept': 'application/vnd.github.v3+json',
      ...
      body: JSON.stringify({
        ...
        head: branchName,
        base: LIVE_EDITOR_CONFIG.defaultBranch
      })
    });
...
*This content was generated by the Live Editor system.*`;
  ...
  async testGitHubToken() {
    ...
    try {
      ...
      const repoResponse = await fetch(`${LIVE_EDITOR_CONFIG.apiBase}`, {
        headers: {
          'Authorization': `token ${token}`,
          'Accept': 'application/vnd.github.v3+json'
        }
      ...
      return {
        ...
        defaultBranch: LIVE_EDITOR_CONFIG.defaultBranch
      };
    } catch (error) {
